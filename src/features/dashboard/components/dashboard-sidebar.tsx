...
import { LayoutDashboard, Video, Bot, Star, CreditCard, LogOut } from "lucide-react";
...
const SideBar = () => {
  ...
  return (
    <aside className="hidden md:flex h-screen w-64 flex-col justify-between border-r border-sidebar-border bg-sidebar text-sidebar-foreground sticky top-0 transition-colors duration-200">
      
      <div className="flex flex-col flex-1 min-h-0">
        <div className="p-4 flex flex-row items-center gap-3 border-b border-sidebar-border/40">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 border border-primary/20 shrink-0">
            <Image src="/logo.svg" width={24} height={24} alt="AgentMeet Logo" loading="eager"/>
          </div>
        ...
        {!isPending && session?.user && (
          <DropdownMenu>
            ...
            <DropdownMenuContent 
              align="end" 
              side="top" 
              className="w-60 border-sidebar-border bg-sidebar text-sidebar-foreground mb-2 shadow-xl p-1.5 rounded-xl animate-in slide-in-from-bottom-2 duration-150"
            >
