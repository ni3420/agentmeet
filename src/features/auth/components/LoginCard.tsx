...
type LoginCardFormValues = z.infer<typeof loginSchema>;
...
const LoginCard = () => {
  ...
  const onSubmit = async (values: LoginCardFormValues) => {
    setIsPending(true);
    setError(null);
    try {
        ...
        <div className="p-6 md:p-8 space-y-6 bg-card">
          <div className="flex flex-col items-center text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Login to your AgentMeet account
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button type="submit" className="w-full mt-2" disabled={isPending}>
              {isPending ? <Loader className="size-4 animate-spin" /> : "Login"}
            </Button>
          </form>
          ...
          <div className="text-center text-sm text-muted-foreground">
            ...
            <Link href="/sign-up" className="underline underline-offset-4 font-medium text-foreground hover:text-primary">
              Sign up
            </Link>
          </div>
        </div>
